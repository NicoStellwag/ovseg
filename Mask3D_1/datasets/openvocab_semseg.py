...
class OpenVocabSemanticSegmentationDataset(Dataset):
    """
    Copy of Mask3D 'SemanticSegmentationDataset' that ...
        - returns CLIP vectors instead of labels.
        - uses GT instances generated by normalized cut instead of dataset GT.
    ONLY SCANNET SUPPORTED FOR NOW
    DOES NOT SUPPORT CACHING FOR NOW
    """
    ...
    def __init__(
        ...
        self.point_instances_file = point_instances_file
        self.instance_labels_file = instance_labels_file
        ...
        # load raw representation on disk to ram if specified
    ...
    def __getitem__(self, idx: int):
        ...
        pointwise_instances = np.argmax(pointwise_instances, axis=1)
        pointwise_instances[no_instance_points] = -1

        # replace dataset gt instances with the ones generated by normalized cut
        labels[:, 1] = pointwise_instances

        # set semantic classes to NaN to make sure code that wants to use them fails
        #labels[:, 0] = np.NaN
        # ===========================
        ...
        norm = self.normalize_color(image=pseudo_image)
        color = np.squeeze(self.normalize_color(image=pseudo_image)["image"])

        #modification: we don't need that anymore since gt generated by
        #normalized cut is already 0 based and consecutive
        #----
        ...
        # unique1 = np.unique(labels[:,0],return_counts=True)

        # # tgt_ids
        # l = labels[:, 0]
        # l_ = l[:,None]
        # indices__ = np.array(VALID_CLASS_IDS_200) == l_
        # indices_ = np.concatenate((np.zeros((indices__.shape[0],1)),indices__),axis=1)

        # a = np.argmax(indices_,axis=1) - 1
        # unique5 = np.unique(a,return_counts=True)
        # nz = np.nonzero(indices__)[0]
        # unique6 = np.unique(nz,return_counts=True)
        # nz_ = np.nonzero(indices__.astype(np.int32))[0]
        # unique7 = np.unique(nz_,return_counts=True)
        

        # mask = a != -1
        # labels[:,0][mask] = a[np.nonzero(indices__)[0]]
        # nonzero_count = indices__.shape[0] - np.nonzero(indices__)[1].shape[0]
        # labels[:,0][~mask] = np.full((nonzero_count),255,dtype=np.float32)
        # unique8 = np.unique(labels[:,0],return_counts=True)
    
        # add segments to labels => labels now (segmentation class, instance, segment)

...
class OpenVocabInstanceSegmentation(pl.LightningModule):
    def __init__(self, config):
        ...
        weight_dict = {
            "loss_cos": matcher.cost_class,
            "loss_mask": matcher.cost_mask,
            "loss_noise_robust": matcher.cost_noise_robust,
        ...
        self.bbox_gt = dict()
        self.agnostic_bbox_preds = dict()
        self.agnostic_bbox_gt = dict()
    ...
    def training_step(self, batch, batch_idx):
        ...
        #instance_tensors = [torch.tensor(target[0]["instance_feats"][i],device="cuda", dtype=torch.float32) for i in range(len(target))]
        #tensor_from_lists = torch.cat(instance_tensors, dim=0)
        ...
        try:
            output = self.forward(
                ...
                raw_coordinates=raw_coordinates,
                #instance_feats=tensor_from_lists
            )
            ...
            if (
                ...
                raise run_err
        #losses = self.criterion(output, target, mask_type=self.mask_type,coords=data.coordinates)
        try:
            losses = self.criterion(output, target, mask_type=self.mask_type,coords=data.coordinates)
        ...
        logs["train_mean_loss_cos"] = statistics.mean(
            [item for item in [v for k, v in logs.items() if "loss_cos" in k]]
        )
    ...
    def save_visualizations(
        ...
        v.save(
            f"{self.config['general']['save_dir']}/visualizations/{file_name}",port=8080
        )

    def eval_step(self, batch, batch_idx):
        data, target, file_names = batch
        inverse_maps = data.inverse_maps#map points to voxelized
        ...
        if self.config.data.test_mode != "test":
            ...
            try:
                losses = self.criterion(
                    output, target, mask_type=self.mask_type,coords=data.coordinates
                )
            except ValueError as val_err:
                print(f"ValueError: {val_err}")
                print(f"data shape: {data.shape}")
    ...
    def eval_instance_step(
        ...
        logit_scale = torch.tensor(np.log(1 / 0.07),requires_grad=False).exp()
        label_offset = self.validation_dataset.label_offset
        
        query_clip_feats = torch.load("/mnt/hdd/clip_query_features/clip_query_features_2.pt").to(torch.float32)#[:20]
        #nn = query_clip_feats.norm(dim=-1, keepdim=True)
        prediction = []
        for i in range(len(output["aux_outputs"])):
            prediction.append(output["aux_outputs"][i])
            normed = (output["aux_outputs"][i]["pred_logits"] / output["aux_outputs"][i]["pred_logits"].norm(dim=-1, keepdim=True))
            max_ = torch.max(normed,dim=-1)
            max_ = torch.max(normed,dim=-2)
            label_logits = normed @ query_clip_feats.T
            prediction[-1]["label_logits"] = label_logits
        #nn = output["pred_logits"].norm(dim=-1, keepdim=True)
        normed = torch.nn.functional.normalize(output["pred_logits"], dim=-1).to(torch.float32)
        query_clip_feats = torch.nn.functional.normalize(query_clip_feats, dim=-1).to(torch.float32)
        #model = self.model
        label_logits = normed @ query_clip_feats.T# * torch.exp(self.criterion.temperature)
        ...
        #     prediction[self.decoder_id]["label_logits"], dim=-1
        # ) [
        #      ..., :-1
        #  ]
        
        
        mi = torch.min(prediction[self.decoder_id][
            "label_logits"
        ],dim=-1)
        ma = torch.max(prediction[self.decoder_id][
            "label_logits"
        ],dim=-1)
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            if not first_full_res:
                ...
                if self.config.general.use_dbscan:
                    ...
                    scores, masks, classes, heatmap = self.get_mask_and_scores(
                        prediction[self.decoder_id]["label_logits"][bid]
                        .detach()
                        .cpu(),
                        masks,
                        prediction[self.decoder_id]["label_logits"][bid].shape[
                            0
                        ],
                        200 - 1,
                    )
        ...
        if self.validation_dataset.dataset_name == "scannet200" or self.validation_dataset.dataset_name == "openvocab_scannet200":
        ...
        for bid in range(len(prediction[self.decoder_id]["pred_masks"])):
            ...
            if (
                ...
                self.agnostic_bbox_preds[file_names[bid]] = [[0] + bbox_data_[1:] for bbox_data_ in bbox_data]
                ...
                self.agnostic_bbox_gt[file_names[bid]] = [[0] + bbox_data_[1:] for bbox_data_ in bbox_data]
    ...
    def eval_instance_epoch_end(self):
        ...
        agnostic_bbox_preds = {scene for scene in self.bbox_preds}
        agnostic_ap_50 = eval_det(
            self.agnostic_bbox_preds, self.agnostic_bbox_gt, ovthresh=0.5, use_07_metric=False
        )
        agnostic_ap_25 = eval_det(
            self.agnostic_bbox_preds, self.agnostic_bbox_gt, ovthresh=0.25, use_07_metric=False
        )
        ...
        ap_results[f"{log_prefix}_agn_val_box_ap_50"] = agnostic_ap_50[
                -1
            ][0]
        ap_results[f"{log_prefix}_agn_val_box_ap_25"] = agnostic_ap_25[
                -1
            ][0]
        root_path = f"eval_output"
        ...
        del self.bbox_preds
        del self.bbox_gt
        del self.agnostic_bbox_preds
        del self.agnostic_bbox_gt
    ...
    def test_epoch_end(self, outputs):
        ...
        dd["val_mean_loss_cos"] = statistics.mean(
            [item for item in [v for k, v in dd.items() if "loss_cos" in k]]
        )

...
class SemanticSegmentationFreeDataset(Dataset):
    ...
    def load_self_train_masks(self, idx, points, freemasks):
        ...
        if not np.allclose(points[:, :3], self_train_cloud[:, :3]):
            # Find KNNs
            self_train_points = self_train_cloud[:, :3]
            tree = KDTree(points)
            dist, ind = tree.query(self_train_points, k=1)
    ...
    def __getitem__(self, idx: int):
        ...
        # Load additional train instances besides the CutLER/FreeMask ones
        ...
        # pcd.colors = o3d.utility.Vector3dVector(np.ones_like(coordinates) * 0.5 * np.any(freemasks, axis=1).reshape(-1, 1).astype(np.float32))

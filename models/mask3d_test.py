...
class Mask3D(nn.Module):
    def __init__(
        ...
        #   ('relu', torch.nn.ReLU()),
        #   ('lin1', torch.nn.Linear(512, 512, bias=True, device="cuda")),
        # ]))
    ...
    def forward(
        ...
        #     target_instance_feats = self.encoder(instance_feats)
        #     target_instance_feats = self.bn(target_instance_feats)
        #     target_instance_feats = self.sig(target_instance_feats)
        ...
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                #stacked = torch.stack(outputs_mask)
                #for i in range(len(outputs_mask)):

                #features = [self.encoder(outputs_mask[i]) for i in range(len(outputs_mask))]

                predictions_class.append(output_class)
                predictions_mask.append(outputs_mask)
                #predictions_mask_1.append(features)
        ...
        return {
            ...
            #"target_instance_feats": target_instance_feats,
            "pointwise_features": mask_features,
            "pred_logits": predictions_class[-1],
            "pred_masks": predictions_mask[-1],
            #"pred_masks_1": predictions_mask_1[-1],
            "aux_outputs": self._set_aux_loss(
                predictions_class, predictions_mask#, predictions_mask_1
            ),
    ...
    @torch.jit.unused
    def _set_aux_loss1(self, outputs_class, outputs_seg_masks, outputs_seg_masks_1):
        # this is a workaround to make torchscript happy, as torchscript
        # doesn't support dictionary with non-homogeneous values, such
        # as a dict having both a Tensor and a list.
        return [
            {"pred_logits": a, "pred_masks": b, "pred_masks_1": c}
            for a, b, c in zip(outputs_class[:-1], outputs_seg_masks[:-1], outputs_seg_masks_1[:-1])
        ]
    @torch.jit.unused

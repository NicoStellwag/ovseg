...
def main(cfg):
    ...
    for i_scene, sample in enumerate(loader):
        ...
        # nr_instances = torch.unique(segment_ids)
        # if(len(nr_instances) > max_instance_count):
        #     max_instance_count = len(nr_instances)
        ...
        # #         coords.cpu().numpy(),
        # #         pointwise_instances,
        # #         cfg.ncut.save_dir + "/" + scene_name + "_" + str(cfg.ncut.affinity_tau * 100).replace("0.","") + "_" + str(cfg.ncut.max_number_of_instances) + ".html",
        # # )

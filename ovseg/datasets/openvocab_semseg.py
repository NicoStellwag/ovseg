...
class OpenVocabSemanticSegmentationDataset(Dataset):
    ...
    Copy of Mask3D 'SemanticSegmentationDataset' that ...
        ...
        - for training, uses GT instances generated by normalized cut instead of dataset GT.
    ...
    def __init__(
        ...
        self.natlang_category = natlang_category
        ...
        if self.natlang_category:
            text_labels = [
                "a " + i["name"] + " in a scene" for i in self._labels.values()
            ]
        else:
    ...
    def map2features(self, labels, device):
        """
        map a list or 1 dim np array or tensor of semantic labels sh(n_labels,)
    ...
    def randfeats(self, n, device):
        """
        random label feats for testing
        """
        feats = []
        for _ in range(n):
            k = choice(list(self.label_feature_map.keys()))
            feats.append(torch.from_numpy(self.label_feature_map[k]).float())
        return torch.stack(feats).to(device)
    ...
    def __getitem__(self, idx: int):
        ...
        # ===========================
        # load instance-wise feature vecs
        ...
        # this is necessary even for validation (where we use ground truth instances),
        # because validation clip vecs are indexed by instance label ids
        # (for validation mode, the ncut script merely maps instance labels
        # to a 0-based consecutive space)
        ...
        # in train mode delete semantic labels to make sure faulty code will fail
        # in validation mode we need them though
        ...
        # modification: only remap labels for validation mode
        # for train mode we set instance labels to NaN anyway
        # ----

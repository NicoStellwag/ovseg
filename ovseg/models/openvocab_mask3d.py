...
class Mask3D(nn.Module):
    def __init__(
        ...
        random_query_both,
        random_normal,
        dropout_before_instance_feature_mlp,
    ):
        super().__init__()
        ...
        if (
            not dropout_before_instance_feature_mlp
            or dropout_before_instance_feature_mlp == 0.0
        ):
            self.class_embed_head = nn.Linear(hidden_dim, self.num_classes)
        else:
            self.class_embed_head = nn.Sequential(
                nn.Dropout(p=dropout_before_instance_feature_mlp, inplace=False),
    ...
    def forward(self, x, point2segment=None, raw_coordinates=None, is_eval=False):
        ...
        for decoder_counter in range(self.num_decoders):
            ...
            for i, hlevel in enumerate(self.hlevels):
                ...
                # FFN
                queries = self.ffn_attention[decoder_counter][i](output).permute(
                    (1, 0, 2)
                )

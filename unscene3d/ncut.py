...
def maskcut3d(aggregated_features, unique_segments, seg_connectivity,
...
def segment_scene(coords, feats, batch, model, config, dataset, segment_ids, seg_connectivity, visualize=False, filter_large_segments=False):
    ...
    bipartitions = maskcut3d(aggregated_features, unique_segments, seg_connectivity, segment_ids, coords[:, 1:],  feats,
...
def main(config):
    ...
    for batch_id, batch in enumerate(tqdm_loader):
        ...
        all_cloud_save = full_res_coords.astype(np.single)  # np.concatenate((full_res_coords, full_res_feats, full_res_labels[:, None], full_res_instance_ids[:, None], full_res_segment_ids[:, None]), axis=1).astype(np.single)
        np.save(f'{freemask_save_base}/{scene_name[0]}_cloud.npy', all_cloud_save)

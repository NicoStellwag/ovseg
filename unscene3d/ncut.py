    ...
    if os.path.isfile(scene_path):
        ...
        if use_image_data:
            ...
            scene_key_feats = torch.zeros(
                (sinput.C.shape[0], model.feature_dim), device=device
            )
            ...
            if config.image_data.load_precomputed:
                ...
                if config.image_data.dino_vit_feature == "attention":
                    ...
                    for i, img in enumerate(images[0]):
                        key_features, query_features = model(
                            img.unsqueeze(0).unsqueeze(0)
                        )  # (batch, img_num, c, h, w)
                        projected_key_features, _ = feature_projecter(
                            encoded_2d_features=key_features,
                            coords=sinput.C,
                            view_matrix=camera_poses[0, i].unsqueeze(0).unsqueeze(0),
                            intrinsic_params=color_intrinsics,
                        )
                        projected_query_features, mapping2dto3d_num = feature_projecter(
                            encoded_2d_features=query_features,
                            coords=sinput.C,
                            view_matrix=camera_poses[0, i].unsqueeze(0).unsqueeze(0),
                            intrinsic_params=color_intrinsics,
                        )
                        ...
                        scene_query_feats[projected_points] = torch.stack(
                            (
                                scene_query_feats[projected_points],
                                projected_query_features[projected_points],
                            )
                        ).mean(0)
...
def main(config):
    ...
    for batch_id, batch in enumerate(tqdm_loader):
        ...
        coords, feats, target, instances, segment_ids, seg_connectivity = (
            coords.to(device),
            feats.to(device),
            target.to(device),
            instances.to(device),
            segment_ids[:, config.freemask.segment_dimension_order].to(device),
            torch.from_numpy(
